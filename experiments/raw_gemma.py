...
def main():
    ...
    prompt = """You are presented with a client who listens to the following artists:

{seeds}

You are asked to use your expert knowledge of musical artists and your complete understanding of the user's favorite artists to rank the following artists (on which you are also an expert) in order from most recommended to least recommended:
{candidates}
...
Provide me with only the ranked list of integer IDs associated with the ranking, do not enumerate your thought process."""
    ...
    system_prompt = """You are an expert in music recommendation. Your specialty is in ranking a list of artists based on the user's preferences and existing favorite artists.
You are able to perfectly rank these artists in order of preference (greatest to least), where preference is defined by how much a user with the listed favorite artists will enjoy that artist."""

    name = "Raw Recommendations from Gemma"

    gw = GemmaWrapper(gemma_model, gemma_tokenizer)
    gemma = InternalRecommender(artists, prompt, gw, system_prompt=system_prompt)
